"""
Strands Agent for Bedrock AgentCore Runtime with S3 skill auto-loading.
On cold start, agents fetch their relevant skills from S3 and inject them
into the system prompt. Skills are also available as tools for on-demand loading.
"""
import os
import sys
import time
...
# Override these in generated entrypoints
BASE_SYSTEM_PROMPT = os.environ.get("AGENT_SYSTEM_PROMPT", "You are a helpful AI assistant.")
RELEVANT_SKILLS = []  # e.g. ["game-player-retention", "user-behavior-funnel"]
...
def load_skill(skill_name: str) -> str:
    ...
    Args:
        skill_name: The skill folder name, e.g. 'game-player-retention', 'global-marketing'
...
def invoke(payload):
    ...
    if _agent is None:
        ...
        logger.info(f"Model: {model_id}, Region: {region}")
        ...
        logger.info(f"Agent initialized in {init_duration:.2f}s")
    else:
